...
def confirm(question: str, default_yes=True) -> bool:
    return arrow_menu(question, ["✓ Yes", "✗ No"]) == 0

def confirmAudio(question: str, default_no=True) -> int:
    return arrow_menu(question, ["✓ From video", "✓ Other file", "✗ No"])

def confirmJp2a(question: str, default_no=True) -> int:
    return arrow_menu(question, ["✓ Yes", "✗ No"]) == 0

def confirmSize(question: str, default_no=True) -> int:
    return arrow_menu(question, ["Default", "Set width and height", "Use terminal zoom", "Fullscreen", "Smart full size"])

def confirmColors(question: str, default_no=True) -> int:
    return arrow_menu(question, ["True colors", "Black and white", "Manual config"])
# ─── Banner helper ────────────────────────────────────────────────────────────
