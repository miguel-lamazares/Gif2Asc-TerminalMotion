...
if input_file.endswith(".jpeg" or ".jpg" or ".svg") is True:
    ...
    if song_choice == 1:
        asc.typewrite(asc.Colors.GREEN + "What's the song's address?", 0.02)
        song_path = input("-> ")
    ...
    song_choice = asc.read_int(1, 2)
    if song_choice == 1:
        asc.typewrite(asc.Colors.GREEN + "What's the song's address? (must be wav or wav)", 0.02)
        song_path = input("-> ")
        if not os.path.exists(song_path):
            asc.typewrite(asc.Colors.RED + "File not found!", 0.02)
            exit()
    ...
    asc.typewrite(asc.Colors.GREEN + "Do you wanna play some audio? (1 - yes / 2 - no: )", 0.02)
    song_choice = asc.read_int(1, 2)
    if song_choice == 1:
            asc.typewrite(asc.Colors.GREEN + "Do you wanna play the itself audio or by other file?", 0.02)
            song_choice = asc.read_int(1, 2, "1 - itself audio / 2 - other file: ")
            if song_choice == 1:
